...
void populateHashTable(const string &checkFiles, int index, int fileSize, vector<string> &files, Hash_312 &h);
...
int main() {
    ...
    for (unsigned int i = 2; i < files.size(); i++) {
        cout << i << " " << files[i] << endl;
            populateHashTable(files[i], i, files.size(), files, h);

    }

    h.countSimilarItems(files.size(), files);
  ...
  //  vector<string> *chunks;
    ...
    while (file >> word) {
        ...
        if (num < 6) {                                //start by making chunk of first six words
            ...
            if (num == 6) {                           //first chunk of six words
